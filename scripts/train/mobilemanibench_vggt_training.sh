...
echo "  video_decoder=3x residual(256) -> four learned 2x stages + zero-init latent pyramid injection"
echo "  rgb_path=four-level residual pyramid -> space-to-depth learned residual downsample"
echo "  video_losses=Charbonnier + 0.1 LPIPS + 0.2 SSIM + 0.1 spatial-gradient + 0.05 Laplacian + 0.1 temporal-difference"
...
echo "  geometry_fusion=layer11 fine + layer23 learned coarse adapter"
echo "  geometry_auxiliary_masks=inside metric grid"
